...
class VoxelSetAbstractionJointTraining(nn.Module):
    def __init__(self, model_cfg, voxel_size, point_cloud_range, num_bev_features=None,
                 num_rawpoint_features=None, domains=None, **kwargs):
        super().__init__()
        ...
        if 'raw_points' in self.model_cfg.FEATURES_SOURCE:
            for domain in domains:
                SA_rawpoints, cur_num_c_out = pointnet2_stack_modules.build_local_aggregation_module(input_channels=num_rawpoint_features[domain] - 3, config=SA_cfg['raw_points'][domain])
                setattr(self, 'SA_rawpoints_'+domain, SA_rawpoints)
    ...
    def interpolate_from_bev_features(self, keypoints, bev_features, batch_size, bev_stride, domain=None):
        """
        Args:
            keypoints: (N1 + N2 + ..., 4)
        ...
        """
        x_idxs = (keypoints[:, 1] - self.point_cloud_range[domain][0]) / self.voxel_size[domain][0]
        y_idxs = (keypoints[:, 2] - self.point_cloud_range[domain][1]) / self.voxel_size[domain][1]
    ...
    def sectorized_proposal_centric_sampling(self, roi_boxes, points):
        ...
        try:
            ...
            print(points)
            print(roi_boxes)
            # distance = (points[:, None, :] - roi_boxes[None, :, 0:3]).norm(dim=-1)
            # min_dis, min_dis_roi_idx = distance.min(dim=-1)
            # roi_max_dim = (roi_boxes[min_dis_roi_idx, 3:6] / 2).norm(dim=-1)
            #
            # print(min_dis)
            # print(min_dis + roi_max_dim)
            # print(min_dis.min())
            # print(min_dis.min() + roi_max_dim)
            print(self.model_cfg.SPC_SAMPLING.SAMPLE_RADIUS_WITH_ROI)
            print(self.model_cfg.SPC_SAMPLING.get('NUM_POINTS_OF_EACH_SAMPLE_PART', 200000))
            print(self.model_cfg.NUM_KEYPOINTS)
            print(self.model_cfg.SPC_SAMPLING.NUM_SECTORS)
        return sampled_points
    ...
    def get_sampled_points(self, batch_dict):
        ...
        if self.model_cfg.POINT_SOURCE == 'raw_points':
            ...
            src_points = common_utils.get_voxel_centers(
                ...
                downsample_times=1,
                voxel_size=self.voxel_size[batch_dict['domain']],
                point_cloud_range=self.point_cloud_range[batch_dict['domain']]
            )
    ...
    def forward(self, batch_dict):
        ...
        if 'bev' in self.model_cfg.FEATURES_SOURCE:
            point_bev_features = self.interpolate_from_bev_features(
                ...
                bev_stride=batch_dict['spatial_features_stride'], domain=batch_dict['domain']
            )
        ...
        if 'raw_points' in self.model_cfg.FEATURES_SOURCE:
            ...
            pooled_features = self.aggregate_keypoint_features_from_one_source(
                batch_size=batch_size, aggregate_func=getattr(self, 'SA_rawpoints_' + batch_dict['domain']),
                xyz=raw_points[:, 1:4],
            ...
            if batch_dict['pfe_raw_point_alignment']:
                target_domain = batch_dict['pfe_raw_point_alignment_target_domain']
                target_domain_input_channels = self.num_rawpoint_features[target_domain]

                target_domain_rawpoints = raw_points[:, :1+target_domain_input_channels]
                target_domain_pooled_features = self.aggregate_keypoint_features_from_one_source(
                    batch_size=batch_size, aggregate_func=getattr(self, 'SA_rawpoints_' + target_domain),
                    xyz=target_domain_rawpoints[:, 1:4],
                    xyz_features=target_domain_rawpoints[:, 4:].contiguous() if target_domain_rawpoints.shape[1] > 4 else None,
                    xyz_bs_idxs=target_domain_rawpoints[:, 0],
                    new_xyz=new_xyz, new_xyz_batch_cnt=new_xyz_batch_cnt,
                    filter_neighbors_with_roi=self.model_cfg.SA_LAYER['raw_points'].get('FILTER_NEIGHBOR_WITH_ROI', False),
                    radius_of_neighbor=self.model_cfg.SA_LAYER['raw_points'].get('RADIUS_OF_NEIGHBOR_WITH_ROI', None),
                    rois=batch_dict.get('rois', None)
                ...
                batch_dict[
                    batch_dict['domain'] + '_pfe_raw_point_align_to_' + target_domain + '_source_tensor'] = pooled_features
                batch_dict[
                    batch_dict['domain'] + '_pfe_raw_point_align_to_' + target_domain + '_target_tensor'] = target_domain_pooled_features
        ...
        for k, src_name in enumerate(self.SA_layer_names):
            ...
            xyz = common_utils.get_voxel_centers(
                ...
                voxel_size=self.voxel_size[batch_dict['domain']], point_cloud_range=self.point_cloud_range[batch_dict['domain']]
            )
            try:
                pooled_features = self.aggregate_keypoint_features_from_one_source(
