...
from pcdet.models.model_utils.joint_training_utils import SeparateBN1d
from spconv.pytorch.modules import is_spconv_module

class SparseSequentialJointTraining(spconv.SparseSequential):
    def forward(self, input, domain_list):
        for k, module in self._modules.items():
            if isinstance(module, SparseBasicBlock) or isinstance(module, SparseSequentialJointTraining):
                input = module(input, domain_list)
            elif isinstance(module, SeparateBN1d):
                input = replace_feature(input, module(input.features, domain_list, input.indices))
            elif is_spconv_module(module):  # use SpConvTensor as input
...
def post_act_block(in_channels, out_channels, kernel_size, indice_key=None, stride=1, padding=0,
    ...
    m = SparseSequentialJointTraining(
        conv,
...
class SparseBasicBlock(spconv.SparseModule):
    ...
    def forward(self, x, domain_list):
        ...
        out = self.conv1(x)
        out = replace_feature(out, self.bn1(out.features, domain_list, out.indices))
        out = replace_feature(out, self.relu(out.features))

        out = self.conv2(out)
        out = replace_feature(out, self.bn2(out.features, domain_list, out.indices))
...
class VoxelResBackBone8xJointTrainingSeparateNorm(nn.Module):
    def __init__(self, model_cfg, input_channels, grid_size, domains, **kwargs):
        super().__init__()
        self.model_cfg = model_cfg
        use_bias = self.model_cfg.get('USE_BIAS', None)
        norm_fn = partial(SeparateBN1d, eps=1e-3, momentum=0.01, domains=domains)
        ...
        self.conv1 = SparseSequentialJointTraining(
            SparseBasicBlock(16, 16, bias=use_bias, norm_fn=norm_fn, indice_key='res1'),
            SparseBasicBlock(16, 16, bias=use_bias, norm_fn=norm_fn, indice_key='res1'),
        )

        self.conv2 = SparseSequentialJointTraining(
        ...
        self.conv3 = SparseSequentialJointTraining(
        ...
        self.conv4 = SparseSequentialJointTraining(
        ...
        self.conv_out = SparseSequentialJointTraining(
            # [200, 150, 5] -> [200, 150, 2]
    ...
    def forward(self, batch_dict):
        ...
        for domain in batch_dict['batch_domains']:
            ...
            if batch_dict.get('conv_input_alignment_targets', None) is not None:
                if domain in batch_dict.get('conv_input_alignment_sources'):
                    target_domain = batch_dict.get('conv_input_alignment_targets')[
                        batch_dict.get('conv_input_alignment_sources').index(domain)]
                    ...
                    target_tensor_after_input_conv = getattr(self, 'conv_input_' + target_domain)(
                        truncated_input_sp_tensor)

                    batch_dict[
                        domain + '_conv_input_align_to_' + target_domain + '_source_tensor'] = tensor_after_input_conv._features
                    batch_dict[
                        domain + '_conv_input_align_to_' + target_domain + '_target_tensor'] = target_tensor_after_input_conv._features
        ...
        x_conv1 = self.conv1(fused_tensor, domain_list)
        x_conv2 = self.conv2(x_conv1, domain_list)
        x_conv3 = self.conv3(x_conv2, domain_list)
        x_conv4 = self.conv4(x_conv3, domain_list)

        # for detection head
        # [200, 176, 5] -> [200, 176, 2]
        out = self.conv_out(x_conv4, domain_list)
