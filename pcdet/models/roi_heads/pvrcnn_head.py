...
class PVRCNNHead(RoIHeadTemplate):
    def __init__(self, input_channels, model_cfg, num_class=1, **kwargs):
        ...
        self.use_amp = model_cfg.get('USE_AMP', False)
    ...
    def roi_grid_pool(self, batch_dict):
        ...
        with autocast(enabled=False):
            global_roi_grid_points, local_roi_grid_points = self.get_global_grid_points_of_roi(
                rois.float(), grid_size=self.model_cfg.ROI_GRID_POOL.GRID_SIZE
            )  # (BxN, 6x6x6, 3)
            global_roi_grid_points = global_roi_grid_points.view(batch_size, -1, 3).float()  # (B, Nx6x6x6, 3)
            ...
            pooled_points, pooled_features = self.roi_grid_pool_layer(
                ...
                features=point_features.float().contiguous(),
            )  # (M1 + M2 ..., C)
