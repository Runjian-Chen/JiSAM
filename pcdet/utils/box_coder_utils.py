...
class ResidualCoder(object):
    ...
    def encode_torch(self, boxes, anchors, use_half = False):
        """
        Args:
        ...
        """
        if use_half:
            anchors[:, 3:6] = torch.clamp_min(anchors[:, 3:6], min=1e-3)
            boxes[:, 3:6] = torch.clamp_min(boxes[:, 3:6], min=1e-3)
        else:
        ...
        zt = (zg - za) / dza
        if use_half:
            dxt = torch.log(torch.clamp(dxg / dxa, min = 1e-4))
            dyt = torch.log(torch.clamp(dyg / dya, min = 1e-4))
            dzt = torch.log(torch.clamp(dzg / dza, min = 1e-4))
