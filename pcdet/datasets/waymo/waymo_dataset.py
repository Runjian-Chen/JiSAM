...
class WaymoDataset(DatasetTemplate):
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None):
        ...
        if self.use_shared_memory:
            try:
                ...
                print('already in shared memory ')
    ...
    def load_data_to_shared_memory(self):
        ...
        for info in cur_infos:
            ...
            if os.path.exists(f"/dev/shm/{sa_key}") or os.path.exists(f"shm://{sa_key}"):
                continue
    ...
    def prepare_data(self, data_dict):
        ...
        if self.training and len(data_dict['gt_boxes']) < 5:
...
def create_waymo_infos(dataset_cfg, class_names, data_path, save_path,
    ...
    waymo_infos_train = dataset.get_infos(
        ...
        sampled_interval=40, update_info_only=update_info_only
    )
...
def create_waymo_gt_database(
    ...
    workers=min(16, multiprocessing.cpu_count()), use_parallel=False, crop_gt_with_tail=False, sampled_interval=1):
    dataset = WaymoDataset(
        dataset_cfg=dataset_cfg, class_names=class_names, root_path=data_path,
        training=False, logger=common_utils.create_logger()
    )
    ...
    if use_parallel:
        dataset.create_groundtruth_database_parallel(
            info_path=train_filename, save_path=save_path, split='train', sampled_interval=sampled_interval,
            used_classes=['Vehicle', 'Pedestrian', 'Cyclist'], processed_data_tag=processed_data_tag,
...
def process_single_sequence(dataset_cfg, class_names, data_path, save_path,
                       raw_data_tag='raw_data', processed_data_tag='waymo_processed_data',
                       workers=1, update_info_only=False):
    dataset = WaymoDataset(
        dataset_cfg=dataset_cfg, class_names=class_names, root_path=data_path,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--sampled_interval', type=int, default=1, help='')
    ...
    if args.func == 'create_waymo_infos':
    ...
    elif args.func == 'process_single_sequence':
        try:
            yaml_config = yaml.safe_load(open(args.cfg_file), Loader=yaml.FullLoader)
        except:
            yaml_config = yaml.safe_load(open(args.cfg_file))
        ...
        dataset_cfg.PROCESSED_DATA_TAG = args.processed_data_tag
        from . import waymo_utils
        sequence_file = '/cpfs01/user/zhangbo/chenrunjian/research/pcdet/segment-9985243312780923024_3049_720_3069_720_with_camera_labels.tfrecord'
        waymo_utils.process_single_sequence(Path(sequence_file), Path('/none/'), 5)
