...
class DataProcessor(object):
    ...
    def boxes_category_mapping(self, data_dict=None, config=None):
        if data_dict is None:
            return partial(self.boxes_category_mapping, config=config)

        class_names = config.CLASS_NAMES
        class_mapping = config.CLASS_MAPPING
        ori_class_names = config.ORI_CLASS_NAMES

        gt_boxes = data_dict['gt_boxes']
        for box_id in range(gt_boxes.shape[0]):
            ori_class_id = gt_boxes[box_id, -1]
            ori_class_name = ori_class_names[int(ori_class_id - 1)]
            updated_class_name = class_mapping[ori_class_name]
            updated_class_id = class_names.index(updated_class_name) + 1
            gt_boxes[box_id, -1] = updated_class_id

        data_dict['gt_boxes'] = gt_boxes
        data_dict['updated_class_names'] = class_names
