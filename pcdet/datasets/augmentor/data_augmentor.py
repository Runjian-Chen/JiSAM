...
class DataAugmentor(object):
    ...
    def disable_augmentation(self, augmentor_configs, disable_joint_sampling_only = False):
        self.data_augmentor_queue = []
        aug_config_list = augmentor_configs if isinstance(augmentor_configs, list) \
            else augmentor_configs.AUG_CONFIG_LIST
        ...
        for cur_cfg in aug_config_list:
            if not isinstance(augmentor_configs, list):
                if cur_cfg.NAME=='gt_sampling' and cur_cfg.NAME in augmentor_configs.DISABLE_AUG_LIST and disable_joint_sampling_only:
                    if isinstance(cur_cfg.DB_INFO_PATH, list) and len(cur_cfg.DB_INFO_PATH) == 2:
                        cur_cfg.DB_INFO_PATH = [cur_cfg.DB_INFO_PATH[0]]
                elif cur_cfg.NAME in augmentor_configs.DISABLE_AUG_LIST:
                    continue
            cur_augmentor = getattr(self, cur_cfg.NAME)(config=cur_cfg)
            self.data_augmentor_queue.append(cur_augmentor)
    ...
    def random_beam_range_jittering(self, data_dict=None, config=None):
        if data_dict is None:
            return partial(self.random_beam_range_jittering, config=config)
        points = data_dict['points']
        xyz = points[:, :3]
        std = config['STD']
        angle_std = config.get('ANGLE_STD', 0)
        # generate noise
