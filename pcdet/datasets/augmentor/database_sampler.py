...
class DataBaseSampler(object):
    def __init__(self, root_path, sampler_cfg, class_names, logger=None):
        ...
        self.debug = sampler_cfg.get('DEBUG', False)
        self.class_mapping = sampler_cfg.get('CLASS_MAPPING', None)
        self.simulation_class_filtering = sampler_cfg.get('SIMULATION_CLASS_FILTERING', None)
        self.nus_class_filtering = sampler_cfg.get('NUS_CLASS_FILTERING', None)
        ...
        for db_info_path in sampler_cfg.DB_INFO_PATH:
            ...
            with open(str(db_info_path), 'rb') as f:
                ...
                for cur_class in class_names:
                    if 'simulation' in str(db_info_path) and self.simulation_class_filtering is not None and cur_class in self.simulation_class_filtering:
                        continue
                    if 'nuscenes_dbinfos_10sweeps' in str(db_info_path) and self.nus_class_filtering is not None and cur_class in self.nus_class_filtering:
                        continue
                    if cur_class in infos.keys():
                        self.db_infos[cur_class].extend(infos[cur_class])

                if self.class_mapping is not None:
                    for k, v in self.class_mapping.items():
                        if 'nuscenes_dbinfos_10sweeps' in str(
                                db_info_path) and self.nus_class_filtering is not None and k in self.nus_class_filtering:
                            continue
                        if k in infos.keys():
                            self.db_infos[v].extend(infos[k])
        ...
        self.warmup = sampler_cfg.get('WARMUP', None)
    ...
    def filter_by_difficulty(self, db_infos, removed_difficulty):
        ...
        for key, dinfos in db_infos.items():
            ...
            new_db_infos[key] = [
                ...
                if ('difficulty' not in info.keys()) or (info['difficulty'] not in removed_difficulty)
            ]
    ...
    def add_sampled_boxes_to_scene(self, data_dict, sampled_gt_boxes, total_valid_sampled_dict, mv_height=None, sampled_gt_boxes2d=None):
        ...
        for idx, info in enumerate(total_valid_sampled_dict):
            ...
            if self.warmup is not None and self.warmup > self.iter_cnt:
                if np.sum(obj_points[:, -1]) == 0:
                    continue

            obj_points_list.append(obj_points)
            valid_boxes.append(sampled_gt_boxes[idx:idx+1])
            valid_names.append(total_valid_sampled_dict[idx]['name'])

        if len(obj_points_list) == 0:
            return data_dict
        obj_points = np.concatenate(obj_points_list, axis=0)
        sampled_gt_names = np.array(valid_names)
        sampled_gt_boxes = np.concatenate(valid_boxes, axis=0)
        ...
        if self.debug:
            gt_boxes[:, -1] = 0
            gt_boxes[-sampled_gt_boxes.shape[0]:, -1] = 1
    ...
    def __call__(self, data_dict):
        ...
        for class_name, sample_group in self.sample_groups.items():
            ...
            if int(sample_group['sample_num']) > 0:
                ...
                min_box_size = 100
                for x in sampled_dict:
                    if min_box_size > x['box3d_lidar'].shape[0]:
                        min_box_size = x['box3d_lidar'].shape[0]

                sampled_boxes = np.stack([x['box3d_lidar'][:min_box_size] for x in sampled_dict], axis=0).astype(np.float32)
