    ...
    for cur_it in range(start_it, total_it_each_epoch):
        ...
        if rank == 0:
            ...
            if isinstance(batch_size, dict):
                batch_size_ = [v for k, v in batch_size.items()]
                batch_size = 0
...
def train_model_joint_training(model, optimizer, train_loader, model_func, lr_scheduler, optim_cfg,
                start_epoch, total_epochs, start_iter, rank, tb_log, ckpt_save_dir, train_sampler=None,
                lr_warmup_scheduler=None, ckpt_save_interval=1, max_ckpt_save_num=50,
                merge_all_iters_to_one_epoch=False, use_amp=False,
    ...
    hook_config = cfg.get('HOOK', None)
    augment_disable_flag = False
    joint_training_disable_flag = False
    ...
    with tqdm.trange(start_epoch, total_epochs, desc='epochs', dynamic_ncols=True, leave=(rank == 0)) as tbar:
        ...
        for cur_epoch in tbar:
            ...
            joint_training_disable_flag, total_it_each_epoch, dataloader_iter, train_loader = disable_joint_training_hook(
                hook_config, train_loader, total_epochs, cur_epoch, joint_training_disable_flag, logger,
                cfg.dist_train, cfg.fix_random_seed, model)
    ...
    if hook_config is not None:
        ...
        if DisableAugmentationHook is not None:
            ...
            if (total_epochs - num_last_epochs) <= cur_epoch and not flag:
                ...
                if 'DATA_AUGMENTOR' in dataset_cfg.keys():
                    ...
                    if DisableAugmentationHook.get('DISABLE_JOINT_SAMPLING', False):
                        for domain in dataloader._dataset.domains:
                            getattr(dataset_cfg, domain).DATA_AUGMENTOR.DISABLE_AUG_LIST = DISABLE_AUG_LIST
                            getattr(dataloader._dataset, domain).data_augmentor.disable_augmentation(getattr(dataset_cfg,domain).DATA_AUGMENTOR, disable_joint_sampling_only = True)
                    else:
                        for domain in dataloader._dataset.domains:
                            getattr(dataset_cfg, domain).DATA_AUGMENTOR.DISABLE_AUG_LIST = DISABLE_AUG_LIST
                            getattr(dataloader._dataset, domain).data_augmentor.disable_augmentation(getattr(dataset_cfg,domain).DATA_AUGMENTOR)

                flag = True
    return flag

def disable_joint_training_hook(hook_config, train_loader, total_epochs, cur_epoch, flag, logger, dist, fix_random_seed, model):
    """
    This hook turns off the joint training during training.
    """
    if hook_config is not None:
        DisableJointTrainingHook = hook_config.get('DisableJointTrainingHook', None)
        if DisableJointTrainingHook is not None:
            num_last_epochs = DisableJointTrainingHook.NUM_LAST_EPOCHS
            target_dataset = DisableJointTrainingHook.TARGET_DATASET
            if (total_epochs - num_last_epochs) <= cur_epoch and not flag:
                train_loader.dataset.disable_joint_training(target_dataset)
                logger.info(f'Disable joint training and target at {target_dataset}')
                flag = True

                # update train_loader
                updated_dataset = train_loader.dataset
                if dist:
                    sampler = torch.utils.data.distributed.DistributedSampler(updated_dataset)
                else:
                    sampler = None
                train_loader = DataLoader(
                    updated_dataset, batch_size=train_loader.batch_size, pin_memory=True, num_workers=train_loader.num_workers,
                    shuffle=(sampler is None), collate_fn=updated_dataset.collate_batch,
                    drop_last=False, sampler=sampler, timeout=0, worker_init_fn=partial(common_utils.worker_init_fn, seed=666 if fix_random_seed else None)
                )

                model.module.conv_input_alignment_sources = None
                model.module.conv_input_alignment_targets = None
                model.module.conv_input_alignment = None

    return flag, len(train_loader), iter(train_loader), train_loader